...
async function handleRequest(req) {
  ...
  // Rate limiter - wrap in try/catch in case GAME_KV not ready
  try {
...
if r.returncode != 0:
    ...
    metadata_d = {'body_part': 'script', 'compatibility_date': '2024-12-18', 'bindings': [{'type': 'kv_namespace', 'name': 'GAME_KV', 'namespace_id': '7714d6c8a40c4a12a3d892ee17a5c8d0'}]}
    ...
    if res.get('success'):
        ...
        r3 = requests.post('https://mafia-game.lionbabycrypto.workers.dev/api/rooms',
                          json={'room_code': 'RLTEST'})
        print(f'POST /api/rooms: {r3.status_code} {r3.text[:200]}')
    else:
