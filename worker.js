// Mafia Game Worker — plain JS, KV-backed
...
async function handleRequest(request) {
  ...
  try {
    ...
    const roomsMatch = path.match(/^\/api\/rooms\/([A-Z0-9]+)$/i);
    ...
    if (path === '/api/player-roles' && request.method === 'GET') {
      const code = url.searchParams.get('room_code') || '';
      const playerId = parseInt(url.searchParams.get('player_id') || '0');
      const authToken = parseAuth(request);
      const raw = await GAME_KV.get('room:' + code);
      if (!raw) return json({ error: 'Room not found' }, 404, origin);
      const state = JSON.parse(raw);
      const p = state.players.find(p => p.id === playerId && p.userId === authToken);
      if (!p) return json({ error: 'Forbidden or not found' }, 403, origin);
      return json({ role: p.role || null }, 200, origin);
    }

    if (path === '/api/assign-roles' && request.method === 'POST') {
      const body = await request.json();
      const raw = await GAME_KV.get('room:' + body.room_code);
      if (!raw) return json({ error: 'Room not found' }, 404, origin);
      const state = JSON.parse(raw);
      ...
      state.nightActions = []; state.dayVotes = [];
      await GAME_KV.put('room:' + body.room_code, JSON.stringify(state));
      return json({ success: true, playerCount: count }, 200, origin);
    }

    if (path === '/api/night-actions' && request.method === 'POST') {
      const body = await request.json();
      const authToken = parseAuth(request);
      const raw = await GAME_KV.get('room:' + body.room_code);
      if (!raw) return json({ error: 'Room not found' }, 404, origin);
      const state = JSON.parse(raw);
      const player = state.players.find(p => p.id === body.player_id && p.userId === authToken);
    ...
    if (path === '/api/process-night' && request.method === 'POST') {
      const body = await request.json();
      const raw = await GAME_KV.get('room:' + body.room_code);
      if (!raw) return json({ error: 'Room not found' }, 404, origin);
      const state = JSON.parse(raw);
      ...
      state.phase = 'day_discussion'; state.nightActions = [];
      await GAME_KV.put('room:' + body.room_code, JSON.stringify(state));
      return json({ killedPlayer: killedName ? { name: killedName } : null, saved }, 200, origin);
    }

    if (path === '/api/day-votes' && request.method === 'POST') {
      const body = await request.json();
      const authToken = parseAuth(request);
      const raw = await GAME_KV.get('room:' + body.room_code);
      if (!raw) return json({ error: 'Room not found' }, 404, origin);
      const state = JSON.parse(raw);
      const player = state.players.find(p => p.id === body.voter_id && p.userId === authToken);
      if (!player) return json({ error: 'Forbidden' }, 403, origin);
    ...
    if (path === '/api/process-day' && request.method === 'POST') {
      const body = await request.json();
      const raw = await GAME_KV.get('room:' + body.room_code);
      if (!raw) return json({ error: 'Room not found' }, 404, origin);
      const state = JSON.parse(raw);
      ...
      state.round++; state.phase = 'night'; state.dayVotes = []; state.nightActions = [];
      await GAME_KV.put('room:' + body.room_code, JSON.stringify(state));
