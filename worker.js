...
var onRequest = async (context) => {
  ...
  try {
    ...
    if (path === "/player-roles" && method === "GET") {
      ...
      if (!raw) return json({ error: "Room not found" }, 404, origin);
      const state = JSON.parse(raw);
      const p = state.players.find((p2) => p2.id === playerId && p2.userId === auth.token);
    ...
    if (path === "/process-night" && method === "POST") {
      ...
      const tally = {};
      kills.forEach((k) => {
        if (k.targetId) tally[k.targetId] = (tally[k.targetId] || 0) + 1;
      });
