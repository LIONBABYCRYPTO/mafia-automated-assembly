// worker-fetch.js
var DEFAULT_HEARTBEAT_TIMEOUT = 6e4;
var GHOST_REMOVE_TIMEOUT = 3e5;
var worker_fetch_default = {
    ...
    try {
      ...
      if (path === "/api/rooms" && method === "POST") {
        ...
        const state = {
          ...
          werewolfChat: [],
          finalWords: {},
          lastWolfKingMark: null,
          shooterRevengePending: null
        };
      ...
      if (path === "/api/night-actions" && method === "POST") {
        ...
        if (body.action_type === "investigate" && body.target_id) {
          ...
          if (target && target.role) {
            ...
            response.investigation = { targetId: target.id, targetName: target.name, isGood: !isBad, result: isBad ? "\u2620\uFE0F Bad" : "\u{1F91D} Good" };
      ...
      if (path === "/api/vote-tally" && method === "GET") {
        ...
        const result = Object.keys(tally).map((id) => ({
          playerId: Number(id),
          playerName: nameMap[Number(id)] || "Unknown",
          votes: tally[id]
...
async function handleGetRoom(path, env, origin) {
  ...
  return json({
    ...
    players: state.players.map((p) => ({
      ...
      alive: p.alive,
      online: p.online && now - (p.lastHeartbeat || 0) < DEFAULT_HEARTBEAT_TIMEOUT,
      role: state.phase !== "lobby" && !p.alive ? p.role : void 0
    })),
    shooterRevengePending: !!state.shooterRevengePending,
...
function getRoleDistribution(state) {
  ...
  state.players.forEach((p) => {
    if (p.role && counts[p.role] !== void 0) counts[p.role]++;
  });
  return counts;
}
...
export {
  worker_fetch_default as default
};
